<template>
  ...
  <main class="max-w-lg mx-auto flex-grow w-9/12">
    <div class="bg-gray-100 p-8 md:p-12 my-5 rounded-lg shadow-2xl flex-grow">
      <section>
        <h3 class="text-blue-900 text-3xl font-medium">{{ $t('Register.title') }}</h3>
      </section>
      <hr class="border-blue-900 border-t-2 w-1/4 mt-3"/>
      <AlertSuccess v-if="successful" :title="$t('Register.congratulations')" :message="$t('Register.successful')"
                    :garnish="message"/>
      <section v-if="!successful" class="mt-10">
        <AlertError :message="message && $t(`error.${ message }`)"/>
        <Form class="flex flex-col" @submit="handleRegister" :validation-schema="schema" v-slot="{ errors }">
          <div class="mb-6 pt-3 rounded bg-gray-200">
            <label class="block text-gray-700 text-sm font-bold md:mb-2 ml-3" for="firstName">{{
                $t('firstName')
              }}</label>
          ...
          <div class="mb-6 pt-3 rounded bg-gray-200">
            <label class="block text-gray-700 text-sm font-bold md:mb-2 ml-3" for="lastName">{{
                $t('lastName')
              }}</label>
    ...
    <div class="max-w-lg mx-auto text-center mt-5 mb-6">
      <p class="text-white">
        ...
        <a href="#" class="font-bold hover:underline" @click.prevent="forgot">{{ $t('Forgot.forgot') }}</a>
      </p>
    </div>
  </main>
  <AuthFooter/>
  ...
  data() {
    const schema = yup.object().shape({
      firstName: yup
          ...
          .min(3, this.$t('minChars', { min: 3 }))
          .max(30, this.$t('maxChars', { max: 30 })),
      lastName: yup
          .string()
